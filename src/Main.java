public class Main {
    ...
    // Работник с МIN ЗП
    public static String findMinSalary(Employee[] wocker) {
        int minSalary = wocker[0].getSalary();
        String minSalaryWocker = "";
        for (Employee value : wocker) {
            if (value != null && minSalary > value.getSalary()) {
                minSalary = value.getSalary();
                minSalaryWocker = value.getFullName();
            }
        }
    ...
    // Работник с МAX ЗП
    public static String findMaxSalary(Employee[] wocker) {
        int maxSalary = wocker[0].getSalary();
        String maxSalaryWocker = "";
        for (Employee value : wocker) {
            if (value != null && maxSalary < value.getSalary()) {
                maxSalary = value.getSalary();
                maxSalaryWocker = value.getFullName();
            }
        }
    ...
    public static void main(String[] args) {
        ...
        // Работник с МIN ЗП
        String minSalary = findMinSalary(wocker);
        System.out.println("Самый МАЛЕНЬКИЙ оклад у " + minSalary);
        System.out.println();

        // Работник с МAX ЗП
        String maxSalary = findMaxSalary(wocker);
